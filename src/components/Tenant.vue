  ...
  <v-data-table
    ...
    <template v-slot:top>
      <v-toolbar flat color="white">
        <v-toolbar-title>CRUD Tenants</v-toolbar-title>
        <v-divider 
          class="mx-4" 
          inset 
          vertical
        ...
        append-icon="search" label="Search Tenant"  
        single-line hide-details></v-text-field>
        <v-spacer></v-spacer>
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on }">
            <v-btn color="primary" dark class="mb-2" v-on="on">New Tenant</v-btn>
          </template>
          <v-card>
            <v-card-title>
...
<script>
    ...
    export default {
        ...
        methods: {
            list(){
                let me = this;
                axios.get('api/Tenants/GetTenants')
                .then(function(response){
                  //console.log(response);
                  me.tenants = response.data;
            ...
            deleteItem (item) {
              let me = this;
              if(confirm('¿Estás seguro que quieres eliminar este Tenant?'))
                axios.delete('api/Tenants/'+item.tenantID,{
                      'id': item.tenantID
                }).then(function(response){
            ...
            save() {
              let me = this;
              if(this.editedIndex > -1) { //Editar Tenant
                axios.put('api/Tenants/PutTenant',{
                ...
                axios.post('api/Tenants',{ // Nuevo Tenant
                           'firstName': me.firstName,
                           'lastname': me.lastname,
                           'email': me.email,
                           'dni': me.dni,
